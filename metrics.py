...
def geometric_mean_score(y_true,
    ...
    .. [2] Barandela, R., SÃƒÆ’Ã‚Â¡nchez, J. S., GarcÃƒâ€žÃ‚Â±a, V., & Rangel, E. "Strategies
...
def make_index_balanced_accuracy(alpha=0.1, squared=True):
    ...
    References
    ----------
    .. [1] GarcÃƒÆ’Ã‚Â­a, Vicente, Javier Salvador SÃƒÆ’Ã‚Â¡nchez, and RamÃƒÆ’Ã‚Â³n Alberto
...
def _prf_divide(numerator, denominator, metric, modifier, average, warn_for):
    """Performs division and handles divide-by-zero cases"""
    result = numerator / denominator
    zero_division_mask = denominator == 0.0
    
    if not np.any(zero_division_mask):
        return result
    
    # Set specific warnings for problematic metrics
    if metric in warn_for:
        warnings.warn(f"Undefined {metric} when {modifier} is 0.0",
                      RuntimeWarning)
    
    # Handle different averaging scenarios
    if average == "binary":
        return 0.0
    elif average is None:
        result[zero_division_mask] = 0.0
        return result
    else:
        if metric in warn_for:
            warnings.warn(f"{metric} ill-defined for some labels",
                          RuntimeWarning)
